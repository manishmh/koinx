...
const AboutBitcoin = ({ coinData }: { coinData: any }) => {
  ...
  const sanitizedHtml = DOMPurify.sanitize(details);
  const [part1, part2, part3] = divideParagraph(sanitizedHtml);
    ...
    <div className="bg-white rounded-lg mt-4 p-4 border shadow-sm md:border-none">
      ...
      <div className="my-4">
        ...
        <p className="text-gray900 mt-2">
          {coinData?.name}&apos;s price today is US ${coinData?.usd}, with a
          24-hour trading volume of $
          {formatLargeNumber(coinData?.usd_market_cap)}. {coinData?.name} is{" "}
          ...
          {coinData?.name} has a circulating supply of 19.24 BTC and a max
          supply of 21 MBTC.
        </p>
        <hr className="mt-4" />
      </div>
      <div>
        ...
        <div className="space-y-6 py-4">
          <p dangerouslySetInnerHTML={{ __html: part1 }}></p>
          <p dangerouslySetInnerHTML={{ __html: part2 }}></p>
          <p dangerouslySetInnerHTML={{ __html: part3 }}></p>
        </div>
        <hr />
        <div className="">
          ...
          <div className="flex gap-8 my-4 flex-wrap">
            <HoldingCoinCard
              classname="from-[#75EBA4] to-[#1667AD]"
              heading="Calculate your Profits"
            />
            <HoldingCoinCard
              classname="from-[#FE9563] to-[#F24039]"
